...
def GenerateRunners(number_runners):
    ...
    for i in range(number_runners):
        ...
        #randomized way to distribute runners throughout the corrals based on the likelihood
        #   of being in a corral (because some corrals have more runners than others)
        while (float(percentage[corralNum]) < float(randNum) and corralNum<12):
            #decide which one to put by looping on increasing number of corrals
            corralNum+=1
            #print corralNum,
        #add a runner to that corral
        ...
        runner = Runner(i,wave,corralNum,runnerStdDev)
        
        Runners[i]=(runner)
    
    
    # corral numbers
    # number of runners in each corral
    print corralCum
        #now give them start times
    #startInterval is interval in which each runner can begin from their starting time
    ...
    for i in range(number_runners):
        runner = Runners[i]
        corral = runner.corral
        #give each runner a slightly different start time by slowing increasing the delay time
        #   as the number of runners we have looked at in this loop increases
        # corralNumDone gradually increases from 0 to number_runners to increase startDelay
...
def main():
    ...
    with con:
        ...
        #divide by ratio_grouped because the simulation has 1 runner represent 40 for example
        ...
        while (minute<500):
            ...
            if droputDone[Halfhour]<dropoutPerHalfHour[Halfhour]:
                #half the time you increase the number of dropouts by 1
                if (random.random()>0.5):
                    drop = 1
                    #find which runner is the one to dropout of the race
            ...
            if drop==1:
                ...
                #label that runner as a dropout
            ...
            started = 0
            #the key is 0 - number_runners
            for key in Runners:
                runner = Runners[key]
                #start the runner if necessary
            ...
            for key in Runners:
                ...
                if runner.finished==True:
                    ...
                    finish=0
                sql = "INSERT into MarathonRunners (runnerId,minute,started,finished,corral,startTime,position,deviation) VALUES (%s, %s, %s, %s, %s, %s,%s, %s)"
                args = (str(runner.id), str(minute), str(start), str(finish), str(runner.corral), str(runner.startTime), str(runner.position), str(runner.stdDev))
                # print sql
                cur.execute(sql, args)
                #operations appears to be a sanity check
                operations+=1
            con.commit()
            ...
            for key in Runners:
                ...
                if runner.started==True:
                    ...
                    # file.write(str(pos)+'\n')
                    for i in range(27):
                        #runner.position is in km so have to convert
                        if float(pos)>float(1.609*float(i)):
                            mileData[i]+=ratio_grouped
            
            inMile = [0]*27
            for i in range(26):
                #find number of people in each mile
